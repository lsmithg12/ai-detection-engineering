"""
Quality Monitor Agent — Reviews deployed detections, tracks health
metrics, and makes tuning/retirement recommendations.
...
AGENT_NAME = "quality"
ALL_AGENTS = ["intel", "red-team", "blue-team", "quality", "security"]
...
def generate_daily_report(
    ...
    report = f"""# Quality Monitor Report — {_today()}
...
"""
    ...
    report += f"\n---\n*Generated by quality agent on {_now_iso()}*\n"
    return report
...
def run(state_manager: StateManager) -> dict:
    ...
    if cross_agent_insights:
        print(f"  [quality] Cross-agent insights:")
        for insight in cross_agent_insights:
            print(f"    - {insight}")
    ...
    if not all_active:
        print("  [quality] No DEPLOYED or MONITORING detections. Nothing to review.")
        return {"summary": "No active detections to review", "detections_reviewed": 0}

    print(f"  [quality] Found {len(all_active)} active detections "
          f"({len(deployed)} DEPLOYED, {len(monitoring)} MONITORING)")

    # 3. Calculate health for each
    results = []
